...
/**
 * Clase utilizada para realizar varias operaciones de formateo de texto, número y fechas.<br>
  * @author: Aitor Bilbao
 * @version: 1.0
 */
 ...
 * La hora está en formato 24 horas.
 */
public JhDate() {
...
/**
 * Construye un objeto jhDate con la fecha pasada por parámetros BigDecimal
...
/**
 * Construye un objeto jhDate con la fecha y hora pasados por parámetros BigDecimal
 * @param year BigDecimal
 * @param month BigDecimal
 * @param day BigDecimal
...
/**
 * Construye un objeto jhDate con la fecha pasada como parámetros.
 * @param year int
 * @param month int
 * @param day int
 */
...
/**
 * Construye un objeto jhDate con la fecha y hora pasados por parámetros int
 * @param year int
 * @param month int
 * @param day int
 * @param hour int
...
/**
 * Construye un objeto jhDate con la hora actual y la fecha pasada como parámetro según el formato indicado
 * @param fecha String
 * @param formato int
 ...
 *		4- aaaamm
 * El String fecha puede contener o no el símbolo "/".
 */
...
public JhDate(String fecha, String separador, int formato) throws Exception{
        ...
        if (fecha.trim().length()!= 6 && fecha.trim().length()!= 8 && fecha.trim().length()!= 10 && fecha.trim().length() > 30)
                throw new Exception("Longitud de fecha no válida");

        switch (formato){
                case 1:{
                        //La fecha está en formato ddmmaaaa
                ...
                case 2:{
                        //La fecha está en formato aaaammdd
                        if (fecha.indexOf(separador) == -1){
                                //No hay barras
                ...
                case 4:{
                        //La fecha está en formato aaaamm
                        if (fecha.indexOf(separador) == -1){
                                //No hay barras
                                ano = new BigDecimal(fecha.substring(0,4)).intValue();
                                mes = new BigDecimal(fecha.substring(4,6)).intValue();
...
public void add(int campo, int incremento){
        try{
                ...
                switch (campo){
                        case 1:{
                                //Añadimos segundos, todavía no lo necesitamos
                                break;
                        }
                        case 2:{
                                //Añadimos minutos
...
/**
 * Compara dos fechas entre sí
 * Parámetros de entrada:
                 - fecha: variable de tipo Date que recibe la función
   Parámetros de salida:
                   - -1: si la fecha es mayor que la pasada por parámetro
                   -  0: si la fecha introducida es igual
                   -  1: si la fecha es menor que la pasada por parámetro
 */
public int equals(JhDate fecha2) {
        ...
        //Si son del mismo año y mes comparamos el día
        if (dia > this.getDay()) return 1;
        if (dia < this.getDay()) return -1;

        //Si son del mismo día comparamos las horas
...
public String getDate(int formato, boolean barras) throws Exception{
        ...
        switch (formato) {
                case 1:{
                        if (barras){
                                fecha += Util.fill(new BigDecimal(this.getDay()).toString(),'0',2,1);
                                fecha += "/";
                                fecha += Util.fill(new BigDecimal(this.getMonth()).toString(),'0',2,1);
                                fecha += "/";
                                fecha += Util.fill(new BigDecimal(this.getYear()).toString(),'0',4,1);
                        }else{
                                fecha += Util.fill(new BigDecimal(this.getDay()).toString(),'0',2,1);
                                fecha += Util.fill(new BigDecimal(this.getMonth()).toString(),'0',2,1);
                                fecha += Util.fill(new BigDecimal(this.getYear()).toString(),'0',4,1);
                ...
                case 4:{
                        ...
                        fecha+= Util.fill(new BigDecimal(this.getDay()).toString(),'0',2,1);
                        fecha+= " ";
                        fecha+= Util.fill(new BigDecimal(this.getHoras()).toString(),'0',2,1);
                        fecha+= ":";
                        fecha+=Util.fill(new BigDecimal(this.getMinutos()).toString(),'0',2,1);
                        fecha+=":";
                        fecha+=Util.fill(new BigDecimal(this.getSegundos()).toString(),'0',2,1);
                        fecha+=".0";
                        break;
                }
        }
...
/**
 * Nos devuelve la hora con descripción larga
 * @return java.lang.String
 * @param formato int
...
public String getHour(int formato) {
        switch (formato){
                case 1:
                {
                        String horas = Util.fill(String.valueOf(this.getHoras()),'0',2,1);
                        String minutos = Util.fill(String.valueOf(this.getMinutos()),'0',2,1);
                        String segundos = Util.fill(String.valueOf(this.getSegundos()),'0',2,1);
                ...
                case 5:{
                        return (Util.fill(String.valueOf(this.getHoras()),'0',2,1)+Util.fill(String.valueOf(this.getMinutos()),'0',2,1));
                }
        }
...
/**
 * Nos devuelve la fecha con descripción larga
 * @return java.lang.String
 * @param formato int
 * El formato puede tener los siguientes valores:
...
/**
 * Valida la fecha
 * @param ano int	Año a validar
 * @param mes int	Mes a validar
 * @param dia int	Día a validar
 * @return boolean Indica si la fecha es válida
 */
...
/**
 * Realiza la validación de las horas.
 * @return boolean	Indica si la hora es válida o no
